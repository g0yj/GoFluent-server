...
public class UserPhoneUpdateService {

  private final JdbcTemplate jdbcTemplate;
  // private final EncryptionService encryptionService;

  @Transactional
  public void updateUserPhones() {
    String csvFilePath = DIR_PATH + "/cellphone_user.csv";
    int resetCount = resetAllPhoneNumbers();
    log.info("모든 사용자의 전화번호를 null로 초기화했습니다. 초기화된 레코드 수: {}", resetCount);
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (values.length == 2) {
          ...
          if (!phoneNumber.equals("없음")) {
            ...
            totalUpdated += updateUserPhone(id, phoneNumber);
          } else {
            totalSkipped++;
          }
        }
  ...
  private int resetAllPhoneNumbers() {
    String sql = "UPDATE user_ SET cell_phone = NULL";
    return jdbcTemplate.update(sql);
  }

  private int updateUserPhone(String id, String encryptedPhoneNumber) {
    String sql = "UPDATE user_ SET cell_phone = ? WHERE id = ?";
